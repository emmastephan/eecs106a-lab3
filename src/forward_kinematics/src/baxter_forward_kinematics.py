...
def baxter_forward_kinematics_from_angles(joint_angles):
	...
	#kfs.homog_3d()
	xi = np.ndarray((6,7))
	cross_result = np.ndarray((3,7))
	for i in range(7):
		cross_result[0:3,i] = -1 * np.cross(ws[0:3,i], qs[0:3,i])
	xi[0:3,0:7] = cross_result
	xi[3:6,0:7] = ws

	G_st0 = np.ndarray((4,4))
	G_st0[0:3, 3] = qs[0:3,7]
	G_st0[3, 0:4] = [0, 0, 0, 1]
	G_st0[0:3, 0:3] = R[0:3, 0:3]
	G_st = np.dot(kfs.prod_exp(xi, joint_angles), G_st0)
