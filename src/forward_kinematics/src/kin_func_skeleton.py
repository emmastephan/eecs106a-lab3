...
def rotation_3d(omega, theta):
    ...
    omega_skew = skew_3d(omega)
    omega_norm = np.linalg.norm(omega)
    R = np.eye(3) + (omega_skew / omega_norm) * np.sin(omega_norm * theta) + (np.dot(omega_skew, omega_skew) / omega_norm ** 2) * (1 - np.cos(omega_norm * theta))
    return R
...
def hat_3d(xi):
    ...
    """
    twist = np.array([[0, -xi[5], xi[4], xi[0]], [xi[5], 0, -xi[3], xi[1]], [-xi[4], xi[3], 0, xi[2]], [0, 0, 0, 0]])
    return twist
...
def homog_3d(xi, theta):
    ...
    if (xi[3] == 0) and (xi[4] == 0) and (xi[5] == 0):
        ret = np.array([[1, 0, 0, xi[0] * theta], [0, 1, 0, xi[1] * theta], [0, 0, 1, xi[2] * theta], [0, 0, 0, 1]])
        return ret
    else:
        ewtheta = rotation_3d(omega, theta)
        calc1 = (1/(norm * norm))
        calc2 = (np.eye(3) - ewtheta)
        calc3 = (np.dot(skew_3d(omega), v_array))
        wwt = np.outer(omega, np.transpose(omega))
        calc4 = np.dot(wwt, (v_array*theta))
        second = calc1 * ((np.dot(calc2, calc3)) + calc4)
        return np.array([[ewtheta[0][0], ewtheta[0][1], ewtheta[0][2], second[0]],
                         [ewtheta[1][0], ewtheta[1][1], ewtheta[1][2], second[1]],
                         [ewtheta[2][0], ewtheta[2][1], ewtheta[2][2], second[2]],
                         [0, 0, 0, 1]])
